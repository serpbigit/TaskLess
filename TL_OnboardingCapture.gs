 ...
 * Receives POST from Cloudflare Worker and appends a row to the target sheet.
 *
 * Expected Worker body shape:
 * {
 *   receivedAt: "2026-03-05T12:34:56.789Z",
 *   ip: "1.2.3.4",
 *   ua: "...",
 *   origin: "https://automatziot.com",
 *   path: "/capture" or "/oauth_callback",
 *   payload: { ... },
 *   enrich:  { ... }
 * }
 */
...
function doPost(e) {
  ...
  return jsonOut_({
    ...
    debug: {
      ...
      enrich_ok: enrichObj && enrichObj.ok === true,
      businesses_count: businessesCount,
      waba_ids: extracted.wabaIds,
      phone_number_ids: extracted.phoneNumberIds,
      display_phone_numbers: extracted.displayPhoneNumbers,
      verified_names: extracted.verifiedNames,
      redirect_uri_used: redirectUriUsed,
      redirect_uri_from_client: redirectUriFromClient,
      payload_redirect_uri: payloadRedirectUri,
      token_exchange_attempted: tokenExchangeAttempted,
      token_present: tokenPresent,
      token_value_present: !!tokenValue,
      meta_error_code: metaErr.code || "",
      meta_error_subcode: metaErr.error_subcode || "",
      meta_error_message: metaErr.message || ""
    }
  });
}
...
function extractIdsFromEnrich_(enrich) {
  ...
  for (var i = 0; i < wabas.length; i++) {
    ...
    for (var j = 0; j < phones.length; j++) {
      ...
      if (p.id) {
        phoneSet[String(p.id)] = true;
      }

      if (p.display_phone_number) {
        displaySet[String(p.display_phone_number)] = true;
      }

      if (p.verified_name) {
        verifiedSet[String(p.verified_name)] = true;
      }
    }
  }
